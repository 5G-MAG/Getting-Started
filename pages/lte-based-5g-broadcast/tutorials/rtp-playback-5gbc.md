...
# Linux stack - RTP Playback over 5G Broadcast
...
the [MBMS Modem](https://github.com/5G-MAG/rt-mbms-modem) and played in a third-party media player such as ffplay or VLC.

## Background

### Real-Time Transport Protocol (RTP)

The Real-Time Transport Protocol (RTP) is a network protocol for delivering audio and video data via unicast or
multicast over IP-based networks.
...
In order to be able to play an RTP stream which is received by 5G Broadcast, we need to install and run the following
components of the 5G-MAG Reference Tools (make sure you have the
suggested [hardware](../additional/hardware-requirements.html)
and [OS](../additional/hardware-requirements.html#os) ready):
...
3. [Download an RTP sample file](../additional/sample-files.html#rtp-payload)
   or setup an RTP live stream on your transmitting infrastructure.
...
4. Start the MBMS Modem with a sample as
   specified [here](https://github.com/5G-MAG/rt-mbms-modem?tab=readme-ov-file#run-a-sample-file). It is
   important to provide the right bandwidth to the MBMS Modem. Consequently, for a 5 MHz bandwidth sample file, the
   command looks like this: `sudo ./modem -f "PathToSample/5MHz_MCS16_1kHz25_RTP_3.5.raw" -b 5`.
...
To identify the correct multicast address for playback, navigate to the terminal in which the modem process is running
and check the log output. Next to the `TMGI` information, the multicast addresses are listed:
...
1. Verify that the correct network interface is used to output the UDP multicast.
   See [Multicast Routing](https://github.com/5G-MAG/rt-mbms-modem?tab=readme-ov-file#multicast-routing)
